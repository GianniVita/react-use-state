...
export default function TabsContainer() {
    const [activeTabId, setActiveTabId] = useState(languages[0].id);

    const activeLanguge = languages.find(lang => loadEnvFile.id === activeTabId);

    const andleTabChange = (id) => {
            ...
            <div className="tabs-buttons">
                {languages.map((lang) => (
                    <TabButton
                        ...
                        title={lang.title}
                        id={lang.id}
                        isActive={lang.id === activeTabId} 

                        onClick={() => handleTabChange(lang.id)} 

                        customStyle={lang.style} 
                    />
                ))}
            </div>
